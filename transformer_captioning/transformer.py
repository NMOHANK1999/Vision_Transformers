...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.bmm(query, key.transpose(1,2)) / torch.sqrt(self.embed_dim)
        ...
        if attn_mask is not None:
            ...
            additive_mask = torch.zeros_like(attn_mask, dtype= torch.float, device= self.device)
            additive_mask[attn_mask == 0.0] = float('-inf')
            #additive_mask[attn_mask != 0.0] = 0.0 # check if this correct
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        assert (num_heads * self.head_dim == self.embed_dim), "Embed dim. needs to be divisible by head number"
        ...
        self.head_proj = nn.Linear(num_heads * self.head_dim, embed_dim)
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        #supposed to permute but lets see if we can use einsum instead
        query = query.reshape(N, S, H, self.head_dim)
        key = key.reshape(N, T, H, self.head_dim)
        value = value.reshape(N, T, H, self.head_dim)
        ...
        dot_product = torch.einsum("nqhd, nkhd -> nhqk", [query, key]) / torch.sqrt(self.head_dim)
        #Query shape = N, S, H, D//H
        #Key Shape, N, T, H, D//H
        ...
        y = torch.einsum("nhql, nlhd -> nqhd", [y, value])

        # concat embeddings from different heads, and project
        #use3d the below shapes for better readablility
        output = y.reshape(N, S, H * self.head_dim)
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        super().__init__()
        # TODO - use torch.nn.Embedding to create the encoding. Initialize dropout layer. max len reps the longest matrix length
    ...
    def forward(self, x):
        ...
        pos = torch.arange(0, S, device= self.device).unsqueeze(0).expand(N, -1)
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        y = self.dropout(self.self_attn(seq, seq, seq, attn_mask=mask))
        out = self.layernorm(y + seq)
        return out
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        y = self.dropout(self.cross_attn(seq, cond, cond))
        out = self.layernorm(y + seq)
        return out
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mask = torch.tril(torch.ones((_len, _len))).cuda()
