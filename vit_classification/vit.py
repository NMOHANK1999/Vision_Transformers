...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.positional_encoding = PositionalEncoding(embed_dim = d_model) # TODO (use the positional encoding from the transformer captioning solution)
        ...
        self.cls_token = nn.Parameter(torch.randn(1, 1, d_model)) # TODO (learnable [CLS] token embedding), repeat this along batches later 
    ...
    def patchify(self, images):
        ...
        #theres 2 ways to do this, 1 is to use a conv layer and rearange it the other way is to use 
        #torch.unfold(dim, windowsize, stride)

        N = images.shape[0]
        patch = images.unfold(2, self.patch_dim, self.patch_dim).unfold(3, self.patch_dim, self.patch_dim)
        images = patch.contigous().view(N, -1, 3 * self.patch_dim * self.patch_dim)
        return images
    ...
    def forward(self, images):
        ...
        output = torch.cat([cls, patches_embedded], dim = 1) # TODO (append a CLS token to the beginning of the sequence of patch embeddings)
        ...
        cls_tokens = output[:, 0, :] # extracts the first thing, from every image
        output = self.fc(cls_tokens) # TODO (take the embedding corresponding to the [CLS] token and feed it through a linear layer to obtain the logits for each class)
